...
int main(int argc, char **argv)    //argc is the cout of argument
{
    int n;
    int yes = 1;
...
//Fill the sructure fields with values
...
//queue upto 5 connnect request
 ...
 printf("The message recieved by client: %s\n",buf);
 write(new_sd,buf,n);
 close(new_sd);
 }
 close(sd);
